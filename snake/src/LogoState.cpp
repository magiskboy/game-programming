#include "../include/LogoState.h"

LogoState::LogoState(Core *_core) : core(_core) {}

LogoState::~LogoState() {}

void LogoState::Init() {
  SDL_Color cl = {255, 255, 255};
  this->logoTexture = SDL_CreateTextureFromSurface(
      core->getRenderer(),
      TTF_RenderText_Blended(core->getFont(), "Snake", cl));
  this->playTexture = SDL_CreateTextureFromSurface(
      core->getRenderer(), TTF_RenderText_Blended(core->getFont(), "Play", cl));
  this->creditTexture = SDL_CreateTextureFromSurface(
      core->getRenderer(),
      TTF_RenderText_Blended(core->getFont(), "Credit", cl));
  this->exitTexture = SDL_CreateTextureFromSurface(
      core->getRenderer(), TTF_RenderText_Blended(core->getFont(), "Exit", cl));
...
void LogoState::Update() {
  ...
  case PLAY:
    SDL_SetTextureColorMod(this->playTexture, 255, 0, 0);
    break;
  case CREDIT:
    SDL_SetTextureColorMod(this->creditTexture, 255, 0, 0);
    break;
  case EXIT:
    SDL_SetTextureColorMod(this->exitTexture, 255, 0, 0);
    break;
  }
}
...
void LogoState::Draw() {
  ...
  SDL_RenderCopy(core->getRenderer(), this->creditTexture, NULL,
                 &this->creditRect);
...
void LogoState::Handle() {
  if (core->getInput()->wasKeyPressed(SDL_SCANCODE_DOWN)) {
    ...
    switch (this->choiceState) {
    case PLAY:
      MachineState::getInstance()->changeState(new PlayState(core));
      break;
    case CREDIT:
      MachineState::getInstance()->changeState(new CreditState(core));
      break;
    case EXIT:
