...
void render_score() {
  ...
  SDL_Surface *surface_score =
      TTF_RenderText_Solid(font, str_score.c_str(), cl);
  SDL_Texture *texture_score =
      SDL_CreateTextureFromSurface(rend, surface_score);
  SDL_Rect frame_score = {SCREEN_WIDTH - surface_score->w - 20, 20,
                          surface_score->w, surface_score->h};
  SDL_RenderCopy(rend, texture_score, NULL, &frame_score);
}

class Doge {
public:
  Doge(const int &n, const int &x, const int &y, const int &rate = 20,
       const int &v = 5) {
...
public:
  ...
  void render() {
    this->frame = {this->x * DOGE_FRAME_SIZE, SCREEN_HEIGHT - 200,
                   DOGE_FRAME_SIZE, 200};
...
int init() {
  ...
  wind = SDL_CreateWindow("Doge War", SDL_WINDOWPOS_CENTERED,
  ...
  for (int i = 0; i < MAX_DOGE; ++i) {
    doges.push_back(
        new Doge(rand() % 5, i * 150, (rand() % (DISTANCE)) - DISTANCE));
  }
...
int update() {
  ...
  for (int i = 0; i < MAX_STAR; ++i) {
    if (stars[i].y > SCREEN_HEIGHT)
      stars[i].y = rand() % DISTANCE - DISTANCE;
    else
      stars[i].y += 5;
  }
