  ...
  void update() {
    if (!this->isDead && SDL_GetTicks() - this->last_update > SPEED) {
      ...
      if (this->blocks[0].x == block_food.x &&
        ...
        this->add_block();
        // generate food
        block_food = {1 + rand() % (MAP_WIDTH - 2),
                      1 + rand() % (MAP_HEIGHT - 2), TILE_SIZE, TILE_SIZE};
      ...
      if (head.x == 0 || head.y == 0 || head.x == MAP_WIDTH - 1 ||
          head.y == MAP_HEIGHT - 1) {
        this->isDead = true;
      }
...
int init() {
  ...
  wind = SDL_CreateWindow(TITLE, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  // generate food
  block_food = {1 + rand() % (MAP_WIDTH - 2), 1 + rand() % (MAP_HEIGHT - 2),
                TILE_SIZE, TILE_SIZE};
  image_food = doge_image[rand() % MAX_IMAGE];
...
int render() {
  ...
  if (!player->isDead) {
    ...
    for (int i = 0; i < MAP_WIDTH; ++i) {
      ...
      // SDL_RenderFillRect(rend, &tile);
      SDL_RenderCopy(rend, doge_image[wall[i + MAP_WIDTH + MAP_HEIGHT]], NULL,
                     &tile);
    }
    ...
    for (int i = 0; i < MAP_HEIGHT; ++i) {
      ...
      // SDL_RenderFillRect(rend, &tile);
      SDL_RenderCopy(rend, doge_image[wall[i + MAP_WIDTH * 2 + MAP_HEIGHT]],
                     NULL, &tile);
    }
